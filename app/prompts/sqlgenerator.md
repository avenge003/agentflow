...
## 注意事项
    ...
    查询结果尽量详细。
    数据库表结构如下： 
    商品流水表：splsk,spid:商品内码,rq:日期(char(10)类型，格式为'YYYY-MM-DD'的字符串，例如'2026-09-08'，不能直接对rq使用YEAR()/FORMAT()函数，日期筛选请用字符串比较，如 rq >= '2026-01-01' AND rq <= '2026-12-31'，或 LEFT(rq,4) = '2026'),dwbh:单位内码,pihao:商品批号,
